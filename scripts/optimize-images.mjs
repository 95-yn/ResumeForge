...
for (const f of files) {
  ...
  if ((meta.width || 0) > 760 || webpBuf.length < src.length * 0.6) {
    process.stdout.write(`  ${f.padEnd(24)} png ${kb(src.length)} → webp ${kb(webpBuf.length)}\n`);
  }
}
...
console.log(`\n✓ 缩略图：${files.length} 张，webp 合计 ${kb(afterTotal)}（原 png 合计 ${kb(beforeTotal)}）`);
